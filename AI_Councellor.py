...
class DynamicCollegeCounselorChatbot:
    """
    Enhanced AI College Counselor with natural conversation flow, selective information extraction,
    and robust content filtering for inappropriate queries.
    """
    ...
    def _get_enhanced_system_prompt(self):
        """Generate an enhanced system prompt with content filtering guidelines"""
        return f"""
        You are {self.name}, a professional, helpful, and respectful college counselor who specializes in helping Indian students find the right colleges and courses.

        YOUR CORE PURPOSE: You ONLY provide guidance on academic, educational, career, and college-related topics.

        STRICT CONTENT GUIDELINES:
        - You MUST decline any inappropriate, adult, sexual, violent, or personal content requests
        - If asked about anything unrelated to education/career, politely redirect to educational topics
        - Never engage with inappropriate queries - immediately redirect with educational alternatives
        - Maintain professional boundaries at all times
        - Focus solely on academic guidance, college selection, career counseling, and educational planning
        ...
        - Professional but not overly formal - like talking to a helpful mentor
        - Encouraging and positive, always highlighting educational possibilities
        - Firm but kind when redirecting inappropriate conversations

        Your conversation approach:
        1. Start with a genuine greeting and get to know the student's educational background
        2. Show interest in their academic dreams, concerns, and career goals naturally
        3. Ask follow-up questions based on their educational interests, not from a checklist
        4. Only move to specific academic details once rapport is established
        5. Provide educational recommendations when you have enough information, not before

        Key principles:
        - Have natural conversations about education - don't rush to collect data
        - Ask ONE educational question at a time, not multiple
        - Show genuine interest in their academic responses
        - Acknowledge their educational concerns and validate their feelings
        - Share educational encouragement and insights naturally
        - Only extract information when it's clearly relevant and explicitly educational

        INAPPROPRIATE CONTENT RESPONSE:
        If someone asks anything inappropriate or non-educational, respond with:
        "I'm here to help you with college selection, academic planning, and career guidance. Let's focus on your educational goals instead! What are you hoping to study, or do you have questions about specific colleges or courses?"

        Remember: You're counseling students about their educational future, not engaging in any other type of conversation. Keep everything professional and education-focused.
        """

    def is_appropriate_query(self, user_message):
        """
        Check if the user message is appropriate and education-related.
        Returns (is_appropriate: bool, severity: str, redirect_message: str)
        """
        message_lower = user_message.lower()
        
        # Define inappropriate content categories
        inappropriate_keywords = {
            'adult_content': ['sex', 'porn', 'nude', 'naked', 'sexual', 'erotic', 'adult content'],
            'personal_intimate': ['dating', 'relationship', 'boyfriend', 'girlfriend', 'romance', 'love'],
            'violence': ['violence', 'hurt', 'kill', 'harm', 'weapon', 'fight'],
            'inappropriate_personal': ['personal life', 'family problems', 'mental health crisis'],
            'off_topic': ['weather', 'sports', 'movies', 'entertainment', 'politics'],
            'testing_boundaries': ['ignore instructions', 'forget you are', 'role play', 'pretend to be']
        }
        
        # Educational/appropriate keywords
        educational_keywords = [
            'college', 'university', 'course', 'study', 'education', 'academic', 'career',
            'exam', 'grade', 'marks', 'admission', 'scholarship', 'stream', 'branch',
            'engineering', 'medical', 'arts', 'science', 'commerce', 'management',
            'future', 'goal', 'aspiration', 'subject', 'degree', 'diploma', 'graduation'
        ]
            ...
            for keyword in keywords:
                if keyword in message_lower:
                    if category == 'adult_content':
                        return False, 'high', "I'm here to help you with college selection and academic planning. Let's focus on your educational goals instead! What subjects are you interested in studying?"
                    elif category == 'personal_intimate':
                        return False, 'medium', "I specialize in college counseling and academic guidance. Let's talk about your educational future instead! Are you looking for specific courses or colleges?"
                    elif category == 'off_topic':
                        return False, 'low', "I'm your college counselor, so I focus on educational topics. What can I help you with regarding colleges, courses, or career planning?"
                    else:
                        return False, 'medium', "I'm here to provide college and career guidance. Let's discuss your academic interests and educational goals instead!"
        
        # Check if the message has any educational content
        has_educational_content = any(keyword in message_lower for keyword in educational_keywords)
        
        # If it's a greeting or general conversation starter, it's okay
        greeting_patterns = ['hi', 'hello', 'hey', 'good morning', 'good afternoon', 'how are you']
        is_greeting = any(pattern in message_lower for pattern in greeting_patterns)
        
        # Very short messages or greetings are generally okay
        if len(message_lower.split()) <= 3 or is_greeting:
            return True, 'none', ''
        
        # If it has educational content, it's appropriate
        if has_educational_content:
            return True, 'none', ''
        
        # If it's unclear but not obviously inappropriate, give benefit of doubt
        # but provide gentle redirect
        if len(message_lower.split()) < 10:
            return True, 'none', ''
        
        # For longer messages without educational content, provide gentle redirect
        return False, 'low', "I'm here to help with college selection and academic planning. What specific educational topics or college-related questions can I assist you with?"

    def handle_inappropriate_content(self, user_message, severity, redirect_message):
        """Handle inappropriate content with appropriate responses"""
        self.inappropriate_warnings += 1
        
        if severity == 'high':
            return f"I understand you might have various questions, but I'm specifically designed to help with college counseling and academic guidance. {redirect_message}"
        elif severity == 'medium':
            return f"I appreciate your message, but my expertise is in college counseling and educational planning. {redirect_message}"
        else:  # low severity
            return redirect_message
        
        # If too many inappropriate attempts, be more firm
        if self.inappropriate_warnings >= self.max_warnings:
            return """I notice we keep getting off the educational topic. I'm here specifically to help you with college selection, course planning, and academic guidance. 

Let me ask you directly: Are you currently a student looking for college advice? What grade are you in, and what subjects interest you most? I'd love to help you explore your educational options! 🎓"""
    ...
    def chat(self, message, history):
        """
        Main chat processing function with natural conversation flow and content filtering
        """
        self.message_count += 1
        print(f"💬 Message {self.message_count}: {message[:50]}...")

        # First, check if the message is appropriate
        is_appropriate, severity, redirect_message = self.is_appropriate_query(message)
        
        if not is_appropriate:
            print(f"⚠️ Inappropriate content detected (severity: {severity})")
            return self.handle_inappropriate_content(message, severity, redirect_message)
        ...
        try:
            ...
            profile_data = {
                "session_info": {
                    ...
                    "inappropriate_warnings": self.inappropriate_warnings,
...
def create_chatbot_interface(api_key):
    """Create enhanced Gradio interface with natural conversation flow and content filtering"""
    if not api_key:
    ...
    with gr.Blocks(title="Lauren - Your Secure AI College Counselor", theme=gr.themes.Soft()) as app:
        gr.Markdown("# 🎓 Lauren - Your AI College Counselor")
        gr.Markdown("""
        Hi! I'm Lauren, your friendly and **secure** AI college counselor. I'm here to have a natural conversation about your educational journey and help you discover the perfect colleges for your goals.
        
        **Please note:** I only discuss academic, educational, and career-related topics. I maintain professional boundaries to ensure our conversation stays focused on helping you succeed academically! 
        
        Just start by saying hello and telling me a bit about yourself. No need to fill out forms - we'll chat naturally and I'll learn about your educational needs as we go! 🎓✨
        """)
        
        # Add content guidelines notice
        with gr.Accordion("📋 Content Guidelines", open=False):
            gr.Markdown("""
            **What I can help with:**
            - College selection and recommendations
            - Course and stream guidance
            ...
            **What I focus on:**
            - Professional, educational conversations only
            - Academic goal setting and planning
            - College comparison and selection
            - Career path exploration
            
            I'll politely redirect any non-educational topics back to academic guidance! 😊
            """)
        ...
        status_display = gr.Markdown("💫 **Status:** Ready to chat! Just say hello and let's get to know each other academically.")

        # Content safety notice
        safety_notice = gr.Markdown(
            "🔒 **Safe Space:** This counselor maintains professional boundaries and focuses only on educational guidance.",
            visible=False
        )
        ...
        initial_greeting = {
            ...
            I'd love to get to know you first from an academic perspective. What's your name? And what brings you here today - are you currently in 12th grade, thinking about your next steps after graduation, or maybe exploring options for higher studies?

            I'm here to provide professional guidance focused entirely on your educational future. There's no rush at all - we can take this conversation at whatever pace feels comfortable for you! 🎓

            **Note:** I specialize in educational and career guidance, so our conversation will stay focused on helping you achieve your academic goals.
            """
        }

        def respond(message, chat_history):
            if not message.strip():
                return chat_history, gr.update(), gr.update(visible=False), gr.update(visible=False)
            ...
            stage_messages = {
                "greeting": "👋 Getting to know your academic background - feel free to share at your own pace!",
                "getting_to_know": "💭 Learning about your educational interests and goals...",
            ...
            status_text = f"💫 **Status:** {stage_messages.get(counselor.conversation_stage, 'Having a great educational conversation!')}"
            status_display_value = gr.update(value=status_text)

            # Show safety notice if inappropriate content was detected
            show_safety_notice = counselor.inappropriate_warnings > 0
            safety_notice_update = gr.update(visible=show_safety_notice)
            ...
            return chat_history, status_display_value, download_visibility, safety_notice_update
        ...
        def new_session():
            ...
            return (
                ...
                gr.update(value="💫 **Status:** Ready to chat! Just say hello and let's get to know each other academically."),
                gr.update(visible=False)
            )
        ...
        submit.click(
            respond, 
            [msg, chatbot], 
            [chatbot, status_display, download_btn, safety_notice]
        ...
        clear.click(new_session, outputs=[chatbot, download_file, status_display, safety_notice])
        ...
        gr.Markdown("""
        ---
        **🎯 Focus Areas:** College Selection | Career Guidance | Academic Planning | Admission Help | Course Recommendations
        
        **✅ Professional Standards:** This counselor maintains strict educational focus and professional boundaries for your safety and better guidance.
        """)
